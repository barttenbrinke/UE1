...
printf "%7s %6s %7s | %s\n" "t(s)" "fps" "ms/frm" "engine ms/100 (illum occl mesh polyv) | occl (clip raster span bounds) | mesh (frame proc light sub clip tmap) | renderer (image complex gouraud tile)"
awk '
  ...
  /PSPPERF:   occlusion:/ { cl=$5; ra=$7; sp=$9; bx=$11; gf=$17 }
  /PSPPERF:   mesh:/ { mp=$5; ml=$7; msb=$9; mc=$11; mt=$13;
     printf "%7.1f %6s %7s | %6s %6s %6s %6s | %6s %6s %6s %5s | %5s %5s %5s %5s %5s %5s | %6s %6s %6s %6s\n", tt, fps, ms, il, oc, me, pv, cl, ra, sp, bx, gf, mp, ml, msb, mc, mt, img, cx, gr, tl;
